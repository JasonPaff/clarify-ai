...
export function registerAiLogsHandlers(
  ...
  ipcMain.handle(IpcChannels.db.aiLogs.create, (_event: IpcMainInvokeEvent, data: NewAiLog): AiLog => {
  ...
  ipcMain.handle(IpcChannels.db.aiLogs.delete, (_event: IpcMainInvokeEvent, id: number): boolean => {
    return aiLogsRepository.delete(id);
  });

  ipcMain.handle(IpcChannels.db.aiLogs.getById, (_event: IpcMainInvokeEvent, id: number): AiLog | undefined => {
  ...
  ipcMain.handle(IpcChannels.db.aiLogs.query, (_event: IpcMainInvokeEvent, params: AiLogFilterParams) => {
    return aiLogsRepository.query(params);
  });

  ipcMain.handle(IpcChannels.db.aiLogs.purge, (_event: IpcMainInvokeEvent, date: string): number => {
    return aiLogsRepository.purgeOlderThan(date);
  });

  ipcMain.handle(IpcChannels.db.aiLogs.getCount, (_event: IpcMainInvokeEvent, filters?: AiLogFilterParams): number => {
    return aiLogsRepository.getCount(filters);
  });

  ipcMain.handle(IpcChannels.db.aiLogs.getLatest, (_event: IpcMainInvokeEvent, limit?: number): Array<AiLog> => {
  ...
  ipcMain.handle(IpcChannels.aiDebugLogging.setConfig, (_event: IpcMainInvokeEvent, config: AiLogConfig): boolean => {
    try {
