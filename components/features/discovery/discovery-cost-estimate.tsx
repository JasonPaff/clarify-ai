...
import { DEFAULT_DISCOVERY_PROMPT } from '@/lib/ai/prompts/discovery';
import { cn } from '@/lib/utils';

/** Cost warning threshold in USD */
const COST_WARNING_THRESHOLD = 0.1;

/** Estimated average output tokens for discovery (file list + analysis) */
...
export const discoveryCostEstimateVariants = cva(
  `
...
interface DiscoveryCostEstimateProps
  extends Omit<ComponentPropsWithRef<'div'>, 'children'>, VariantProps<typeof discoveryCostEstimateVariants> {
  customPrompt?: string;
...
export const DiscoveryCostEstimate = ({
  className,
  customPrompt,
  featureRequest,
  isLoading = false,
  ...
  const estimateData = useMemo(() => {
    ...
    const systemPrompt = customPrompt && customPrompt.trim() ? customPrompt : DEFAULT_DISCOVERY_PROMPT;
    ...
    const totalInputTokens = featureRequestTokens + systemPromptTokens + repositoryOverviewsTokens;
  ...
  }, [customPrompt, featureRequest, modelId, repositoryOverviews]);
  ...
  if (isLoading) {
    ...
    if (isCompactVariant) {
      return (
        <div
          className={cn(discoveryCostEstimateVariants({ status: 'normal', variant: 'compact' }), className)}
          ref={ref}
          {...props}
        >
    ...
    return (
      <div
        className={cn(discoveryCostEstimateVariants({ status: 'normal', variant: 'full' }), className)}
        ref={ref}
        {...props}
      >
  ...
  if (variant === 'compact') {
    const tooltipContent = (
      <div className={'space-y-1'}>
        <div className={'font-medium'}>Discovery Cost Estimate</div>
    ...
    return (
      <Tooltip content={tooltipContent} side={'bottom'}>
        <div
          className={cn(discoveryCostEstimateVariants({ status: statusValue, variant: 'compact' }), className)}
          ref={ref}
          {...props}
        >
  ...
  return (
    <div
      className={cn(discoveryCostEstimateVariants({ status: statusValue, variant: 'full' }), className)}
      ref={ref}
      {...props}
    >
      {/* Header */}
      <div className={'mb-2 flex items-center justify-between'}>
        ...
        <div className={'flex items-center gap-2'}>
          ...
          <span className={'text-sm font-medium'}>Discovery Cost Estimate</span>
        </div>
      ...
      {isWarning && (
        ...
        >
          ...
          <span>
            <strong>High estimated cost.</strong> This discovery may cost more than $0.10. Consider using a smaller
            model or reducing the number of repository overviews.
          </span>
        </div>
      )}
      ...
      <div className={'mt-2 text-xs text-muted-foreground italic'}>
        * Token counts are estimates. Actual usage and cost may vary based on model response and repository complexity.
      </div>
    </div>
  );
};
