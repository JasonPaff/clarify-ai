'use client';

import { AlertDialog } from '@base-ui/react/alert-dialog';
import { useState } from 'react';

import type { FeatureRequest } from '@/db/schema/feature-requests.schema';

import { Button } from '@/components/ui/button';
import { useDeleteFeatureRequest } from '@/hooks/queries/use-feature-requests';
import { useControllableState } from '@/hooks/use-controllable-state';
import { cn } from '@/lib/utils';

type DeleteFeatureRequestDialogProps = Children & {
  featureRequest: Pick<FeatureRequest, 'id' | 'title'>;
  onOpenChange?: (isOpen: boolean) => void;
  open?: boolean;
};

export function DeleteFeatureRequestDialog({
  children,
  featureRequest,
  onOpenChange,
  open: controlledOpen,
}: DeleteFeatureRequestDialogProps) {
  const [isOpen, setIsOpen] = useControllableState({
    defaultValue: false,
    onChange: onOpenChange,
    value: controlledOpen,
  });
  const [confirmationText, setConfirmationText] = useState('');

  const deleteFeatureRequest = useDeleteFeatureRequest();

  const _isConfirmed = confirmationText === featureRequest.title;

  const handleOpenChange = (isDialogOpen: boolean) => {
    setIsOpen(isDialogOpen);
    if (!isDialogOpen) {
      setConfirmationText('');
    }
  };

  const handleDelete = async () => {
    if (!_isConfirmed) return;
    await deleteFeatureRequest.mutateAsync(featureRequest.id);
    setIsOpen(false);
  };

  return (
    <AlertDialog.Root onOpenChange={handleOpenChange} open={isOpen}>
      {/* Trigger */}
      {children && (
        <AlertDialog.Trigger nativeButton={false} render={<span className={'inline-flex'} />}>
          {children}
        </AlertDialog.Trigger>
      )}

      {/* Portal */}
      <AlertDialog.Portal>
        {/* Backdrop */}
        <AlertDialog.Backdrop
          className={cn(
            `
              fixed inset-0 z-50 bg-black/50 backdrop-blur-sm transition-opacity
              duration-200
            `,
            'data-ending-style:opacity-0',
            'data-starting-style:opacity-0'
          )}
        />

        {/* Dialog Content */}
        <AlertDialog.Popup
          className={cn(
            `
              fixed top-1/2 left-1/2 z-50 w-full max-w-md -translate-1/2
              rounded-lg border border-border
            `,
            `
              bg-background p-6 shadow-lg transition-all duration-200
              outline-none
            `,
            'data-ending-style:scale-95 data-ending-style:opacity-0',
            'data-starting-style:scale-95 data-starting-style:opacity-0'
          )}
        >
          {/* Title */}
          <AlertDialog.Title className={'text-lg font-semibold text-foreground'}>
            Delete Feature Request
          </AlertDialog.Title>

          {/* Description */}
          <AlertDialog.Description className={'mt-2 text-sm text-muted-foreground'}>
            Are you sure you want to delete{' '}
            <span className={'font-semibold text-foreground'}>{featureRequest.title}</span>?
          </AlertDialog.Description>

          {/* Warning */}
          <p className={'mt-4 text-sm text-destructive'}>
            This action cannot be undone. The feature request and all associated data will be permanently deleted.
          </p>

          {/* Confirmation Input */}
          <div className={'mt-4'}>
            <label className={'block text-sm text-muted-foreground'} htmlFor={'confirm-delete-feature-request'}>
              Type <span className={'font-semibold text-foreground'}>{featureRequest.title}</span> to confirm
            </label>
            <input
              autoComplete={'off'}
              className={cn(
                'mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground',
                'placeholder:text-muted-foreground',
                'focus:border-accent focus:ring-1 focus:ring-accent focus:outline-none'
              )}
              id={'confirm-delete-feature-request'}
              onChange={(e) => setConfirmationText(e.target.value)}
              placeholder={featureRequest.title}
              type={'text'}
              value={confirmationText}
            />
          </div>

          {/* Actions */}
          <div className={'mt-6 flex justify-end gap-3'}>
            <AlertDialog.Close render={<Button variant={'outline'} />}>Cancel</AlertDialog.Close>
            <Button
              disabled={!_isConfirmed || deleteFeatureRequest.isPending}
              onClick={handleDelete}
              variant={'destructive'}
            >
              {deleteFeatureRequest.isPending ? 'Deleting...' : 'Delete'}
            </Button>
          </div>
        </AlertDialog.Popup>
      </AlertDialog.Portal>
    </AlertDialog.Root>
  );
}
