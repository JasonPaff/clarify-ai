...
export const toastRootVariants = cva(
  `
    ...
    data-starting-style:translate-x-full data-starting-style:opacity-0 data-swipe-direction-left:data-ending-style:-translate-x-full data-swipe-direction-right:data-ending-style:translate-x-full
  `,
  {
    defaultVariants: {
      variant: 'default',
...
export const ToastIcon = ({ className, ref, type, ...props }: ToastIconProps) => {
  ...
  switch (type) {
    ...
    case 'loading':
      return (
        <span ref={ref} {...props}>
          <Loader2 className={cn(iconClasses, 'animate-spin text-muted-foreground')} />
        </span>
      );
    case 'success':
      return (
        <span ref={ref} {...props}>
          <CheckCircle className={cn(iconClasses, 'text-green-600 dark:text-green-400')} />
        </span>
      );
    case 'warning':
      return (
        <span ref={ref} {...props}>
          <AlertCircle className={cn(iconClasses, 'text-amber-600 dark:text-amber-400')} />
        </span>
      );
    default:
      return null;
