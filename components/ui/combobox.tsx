...
export const comboboxPopupVariants = cva(
  `
    max-w-(--available-width) min-w-(--anchor-width) origin-(--transform-origin) rounded-md border
    border-border bg-card p-1 shadow-md
...
export const ComboboxList = ({ className, ref, ...props }: ComboboxListProps) => {
  return (
    <BaseCombobox.List
      className={cn(
        'max-h-[min(23rem,var(--available-height))] overflow-y-auto overscroll-contain py-1 outline-none',
        'scroll-py-1 data-empty:p-0',
        className
      )}
      ref={ref}
      {...props}
...
export const ComboboxEmpty = ({ className, ref, ...props }: ComboboxEmptyProps) => {
  return (
    <BaseCombobox.Empty
      className={cn('p-4 text-sm text-muted-foreground empty:m-0 empty:p-0', className)}
      ref={ref}
      {...props}
    />
  );
...
export const ComboboxStatus = ({ className, ref, ...props }: ComboboxStatusProps) => {
  return (
    <BaseCombobox.Status
      className={cn('flex items-center gap-2 px-2 py-1.5 text-sm text-muted-foreground', className)}
      ref={ref}
      {...props}
    />
  );
...
export const ComboboxChips = ({ className, ref, ...props }: ComboboxChipsProps) => {
  return (
    <BaseCombobox.Chips
      className={cn(
        'flex flex-wrap items-center gap-0.5 rounded-md border border-border px-1.5 py-1',
        'focus-within:ring-2 focus-within:ring-accent focus-within:ring-offset-0 focus-within:outline-none',
        className
      )}
      ref={ref}
      {...props}
...
export const ComboboxChipRemove = ({ children, className, ref, ...props }: ComboboxChipRemoveProps) => {
  return (
    <BaseCombobox.ChipRemove
      className={cn('rounded-sm p-0.5 text-inherit hover:bg-muted', className)}
      ref={ref}
      {...props}
    >
